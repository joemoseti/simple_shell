...
int rellocate_alias(const char *name, const char *val,
		int edit_idx, int cflag, char ***alias);

int edit_alias(char *alias, const char *value, int edit_idx);


/**
 * upd_alias_noval - sets a new
 * environment variable, or updates an existing one.
...
int upd_alias_noval(char **sarr, char ***alias)
{
	...
	for (i = 0; (*alias)[i]; i++)
	{
		...
		{
			/* if name is found */
			return (edit_alias((*alias)[i], "\0", (j + 1)));
		}
	}
	/* if name is not found */
	return (rellocate_alias(sarr[0], "\0", -1, 2, alias));
}
...
/**
 * rellocate_alias - creates a new buffer for storing the alias array.
 * @name: alias name.
 * @val: alias value.
...
/**
 * edit_alias - edits the value of an alias name.
